...
class Animal(Objects):# has World class as an input and the coordinate of the object in that world
    ...
    def reproduce(self):
        ...
        #increment = energy_reDistibution(energy_percentage) - energy_reDistibution(energy_percentage)
            child.speed = self.speed   * speed_mutate 
            child.vision = self.vision * vision_mutate
            child.Size  = self.Size    * size_mutate
            child.width = self.width   * size_mutate#+ increment[2] *self.size_scaling
            child.height = self.height * size_mutate#
            if isinstance(self, Gazelle):
